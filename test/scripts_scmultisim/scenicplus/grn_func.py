...
def _binarize_BASC(
    ...
    context, :class:`pandas.DataFrame` with binarized region-to-gene links.
    """
    from scenicplus.BASCA import binarize
...
def create_emodules(
    ...
    if rho_dichotomize:
        ...
        if keep_only_activating:
            r2g_iter = iter_thresholding(region_to_gene.loc[region_to_gene[CORRELATION_COEFFICIENT_NAME] > rho_threshold], context = frozenset(['positive r2g']))
        else:
            r2g_iter = iter_thresholding(region_to_gene, context = frozenset(['all r2g']))
    ...
    for context, r2g_df in tqdm(r2g_iter, total=total_iter, disable=disable_tqdm):
        for TF_name in tqdm(mtx_cistromes.columns, total=len(mtx_cistromes.columns), desc=f"\u001b[32;1mProcessing:\u001b[0m {', '.join(context)}", leave=False, disable=disable_tqdm):
            # regions_enriched_for_TF_motif = mtx_cistromes.index[mtx_cistromes[TF_name]]
            # r2g_df_enriched_for_TF_motif = r2g_df.loc[list(set(regions_enriched_for_TF_motif) & set(r2g_df.index))]
            # NOTE: temp, remove the filtering step
            r2g_df_enriched_for_TF_motif = r2g_df
...
def build_grn(
    ...
    # TODO: check the function create_emodules
    relevant_tfs, e_modules = create_emodules(
        ...
        n_cpu=n_cpu,
        temp_dir=temp_dir)
    print(f"relevant tfs: {relevant_tfs}")
    print('Subsetting TF2G adjacencies for TF with motif.')
    ...
    if rho_dichotomize_tf2g:
        ...
        # This should never be the case though (if min_target_genes > 1)
        # But better fix this at some point!
        TF_to_ranking = {TF: TF2G_adj_relevant.loc[TF].set_index('target')[order_TFs_to_genes_by].sort_values(ascending = False) for TF in tqdm(TFs, total = len(TFs))}
    ...
    # filter out nans
    new_e_modules_select = []
    for m in new_e_modules:
        if not np.isnan(m.gsea_enrichment_score) and not np.isnan(m.gsea_pval):
            new_e_modules_select.append(m)

    print(
        f'Subsetting on adjusted pvalue: {adj_pval_thr}, minimal NES: {NES_thr} and minimal leading edge genes {min_target_genes}')
    # subset on adj_p_val
    adj_pval = p_adjust_bh([m.gsea_pval for m in new_e_modules_select])
    if any([np.isnan(p) for p in adj_pval]):
        Warning(
            'Something went wrong with calculating adjusted p values, early returning!')
        return new_e_modules_select

    for module, adj_pval in zip(new_e_modules_select, adj_pval):
        module.gsea_adj_pval = adj_pval

    e_modules_to_return: List[eRegulon] = []
    for module in new_e_modules_select:
