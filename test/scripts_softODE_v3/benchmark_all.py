...
def summarize_scores(setting):
    ...
    scores = pd.DataFrame(columns = ["interval", "ngenes", "time", "model", "bandwidth", "truncate_param", "lambda", "nmse","kendall-tau", "pearson", "spearman", "cosine similarity", "AUPRC Ratio (pos)", "AUPRC Ratio (neg)", "AUPRC Ratio (abs)", "Early Precision Ratio (pos)", "Early Precision Ratio (neg)", "Early Precision Ratio (abs)"])
    result_dir = "../results_softODE_v2/" + traj + "_ngenes_" + str(ngenes) + "_ncell_" + str(ntimes) + "_seed_" + str(seed) + "/"
    result_CeSpGRN_dir = "../results_softODE_v2/" + traj + "_ngenes_" + str(ngenes) + "_ncell_" + str(ntimes) + "_seed_" + str(seed) + "/"
    ...
    thetas = np.load(file = result_CeSpGRN_dir + "thetas_" + str(bandwidth) + "_" + str(lamb) + "_" + str(truncate_param) + "_kt.npy")
    ...
    thetas = np.load(file = result_CeSpGRN_dir + "thetas_" + str(bandwidth) + "_" + str(lamb) + "_" + str(truncate_param) + ".npy")
    ...
    # CeSpGRN-kt-TF
    thetas = np.load(file = result_CeSpGRN_dir + "thetas_" + str(bandwidth) + "_" + str(lamb) + "_" + str(truncate_param) +"_" + str(beta) + "_kt.npy")
    ...
    scores.to_csv(result_CeSpGRN_dir + "score_all.csv")
...
def summarize_scores_diff(setting):
    ...
    scores.to_csv(result_CeSpGRN_dir + "score_all_diff.csv")
...
ntimes = 1000
nsample = 1
all_scores = pd.DataFrame(columns = ["interval", "ngenes", "time", "model", "bandwidth", "truncate_param", "lambda", "nmse","kendall-tau", "pearson", "spearman", "cosine similarity", "AUPRC Ratio (pos)", "AUPRC Ratio (neg)", "AUPRC Ratio (abs)", "Early Precision Ratio (pos)", "Early Precision Ratio (neg)", "Early Precision Ratio (abs)"])
for traj in ["linear", "bifurc"]:
    for interval in [20]:
        for (ngenes, ntfs) in [(20, 5)]:
            for seed in [0,1,2,3,4]:
                print("ntimes: " + str(ntimes) + ", interval: " + str(interval) + ", ngenes: " + str(ngenes) + ", seed: " + str(seed) + ", initial graph: sergio")
                result_dir = "../results_softODE_v3/" + traj + "_ngenes_" + str(ngenes) + "_ncell_" + str(ntimes) + "_seed_" + str(seed) + "/"
                score = pd.read_csv(result_dir + "score_all.csv", index_col = 0)
                all_scores = pd.concat([all_scores, score], axis = 0)
...
all_scores = all_scores.groupby(by = ["model", "bandwidth", "truncate_param", "lambda"]).mean()
# all_scores = all_scores.drop(["time"], axis = 1)
display(all_scores)
