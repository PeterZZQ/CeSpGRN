...
for ngenes, ntfs in [(20, 5), (30, 5), (50, 5)]:
    for interval in [50, 100, 200]:
        for stepsize in [0.0001, 0.0002]:
            ...
            score = pd.DataFrame(columns =["model", "kendall-tau","probability of success", "pearson", "cosine similarity", "time"])
            result_dir = "../results_softODE_sergio/results_ngenes_" + str(ngenes) + "_interval_" + str(interval) + "_stepsize_" + str(stepsize) + "/"
            ...
            for time in range(0, ntimes):
                ...
                kt_rand, _ = bmk.kendalltau(G_inf = thetas_rand, G_true = gt_adj[time])
                ...
                score = score.append({"kendall-tau": kt_rand, "probability of success":ps_rand, "pearson": pearson_rand_val, "cosine similarity": cosine_sim_rand, 
                                      "model":"random", "time":time}, ignore_index=True)

            print()
            print("model: random")
            print("mean kendall-tau: " + str(np.mean(score.loc[score["model"] == "random", "kendall-tau"].values)))
            ...
            for time in range(0, ntimes):
                # benchmark genie
                kt,_ = bmk.kendalltau(G_inf = thetas1[time], G_true = gt_adj[time])
            ...
            print("mean kendall-tau: " + str(np.mean(score.loc[score["model"] == "genie_standardscaler", "kendall-tau"].values)))
            ...
            for time in range(0, ntimes):
                kt,_ = bmk.kendalltau(G_inf = thetas2[time], G_true = gt_adj[time])
            ...
            print("mean kendall-tau: " + str(np.mean(score.loc[score["model"] == "genie_standardscaler_tf", "kendall-tau"].values)))
            ...
            for time in range(0, ntimes):
                kt,_ = bmk.kendalltau(G_inf = thetas3[time], G_true = gt_adj[time])
            ...
            print()
            print("model: genie")
            print("mean kendall-tau: " + str(np.mean(score.loc[score["model"] == "genie", "kendall-tau"].values)))
            ...
            for time in range(0, ntimes):
                kt,_ = bmk.kendalltau(G_inf = thetas4[time], G_true = gt_adj[time])
            ...
            print("mean kendall-tau: " + str(np.mean(score.loc[score["model"] == "genie_tf", "kendall-tau"].values)))
            ...
            for time in range(0, ntimes):
                kt,_ = bmk.kendalltau(G_inf = thetas5[time], G_true = gt_adj[time])
            ...
            print("mean kendall-tau: " + str(np.mean(score.loc[score["model"] == "genie_obs", "kendall-tau"].values)))
            ...
            for time in range(0, ntimes):
                kt,_ = bmk.kendalltau(G_inf = thetas6[time], G_true = gt_adj[time])
            ...
            print("mean kendall-tau: " + str(np.mean(score.loc[score["model"] == "genie_obs_tf", "kendall-tau"].values)))
...
for ngenes, ntfs in [(20, 5), (30, 5), (50, 5)]:
    for interval in [50, 100, 200]:
        for stepsize in [0.0001, 0.0002]:
            ...
            ax = fig.add_subplot()
            sns.boxplot(data = score, x = "model", y = "kendall-tau", ax = ax)
            ax.set_title("kendall-tau: ngenes: {:d}, interval: {:d}, stepsize: {:f}".format(int(ngenes), int(interval), stepsize))
            fig.savefig(result_dir + "kendall-tau.png", bbox_inches = "tight")
...
score_models = pd.DataFrame(columns = ["model", "kendall-tau", "pearson", "cosine similarity"])
# for ngenes, ntfs in [(20, 5), (30, 10), (50, 20), (100, 50)]:
for ngenes, ntfs in [(20, 5), (30, 5), (50, 5)]:
    for interval in [50, 100, 200]:
        for stepsize in [0.0001, 0.0002]:
            ...
            score_models = score_models.append({"kendall-tau": score_random["kendall-tau"].mean(), "pearson": score_random["pearson"].mean(), "cosine similarity": score_random["cosine similarity"].mean(), 
                                    "model":"random", "ngenes": ngenes, "interval": interval, "stepsize": stepsize}, ignore_index=True)         
            score_models = score_models.append({"kendall-tau": score_genie_standardscaler["kendall-tau"].mean(), "pearson": score_genie_standardscaler["pearson"].mean(), 
            ...
            score_models = score_models.append({"kendall-tau": score_genie_standardscaler_tf["kendall-tau"].mean(), "pearson": score_genie_standardscaler_tf["pearson"].mean(), 
            ...
            score_models = score_models.append({"kendall-tau": score_genie["kendall-tau"].mean(), "pearson": score_genie["pearson"].mean(), "cosine similarity": score_genie["cosine similarity"].mean(), 
                                    "model":"score_genie", "ngenes": ngenes, "interval": interval, "stepsize": stepsize}, ignore_index=True)    
            score_models = score_models.append({"kendall-tau": score_genie["kendall-tau"].mean(), "pearson": score_genie["pearson"].mean(), "cosine similarity": score_genie["cosine similarity"].mean(), 
                                    "model":"score_genie", "ngenes": ngenes, "interval": interval, "stepsize": stepsize}, ignore_index=True)
            score_models = score_models.append({"kendall-tau": score_genie_tf["kendall-tau"].mean(), "pearson": score_genie_tf["pearson"].mean(), "cosine similarity": score_genie_tf["cosine similarity"].mean(), 
                                    "model":"score_genie_tf", "ngenes": ngenes, "interval": interval, "stepsize": stepsize}, ignore_index=True)
            score_models = score_models.append({"kendall-tau": score_genie_obs["kendall-tau"].mean(), "pearson": score_genie_obs["pearson"].mean(), "cosine similarity": score_genie_obs["cosine similarity"].mean(), 
                                    "model":"score_genie_obs", "ngenes": ngenes, "interval": interval, "stepsize": stepsize}, ignore_index=True)
            score_models = score_models.append({"kendall-tau": score_genie_obs_tf["kendall-tau"].mean(), "pearson": score_genie_obs_tf["pearson"].mean(), "cosine similarity": score_genie_obs_tf["cosine similarity"].mean(), 
...
fig = plt.figure(figsize = (25,7))
ax = fig.add_subplot()
sns.boxplot(data = score_models, x = "model", y = "kendall-tau", ax = ax)
ax.set_title("kendall-tau")
fig.savefig("../results_softODE_sergio/kendall-tau.png", bbox_inches = "tight")       
fig = plt.figure(figsize = (25,7))
ax = fig.add_subplot()
...
score_models = pd.DataFrame(columns = ["model", "kendall-tau","probability of success", "pearson", "cosine similarity", "time", "ngenes", "interval", "stepsize"])
# for ngenes, ntfs in [(20, 5), (30, 10), (50, 20), (100, 50)]:
for ngenes, ntfs in [(20, 5), (30, 5), (50, 5)]:
    for interval in [50, 100, 200]:
        for stepsize in [0.0001, 0.0002]:
...
score_models_genie_standardscalar = score_models[score_models["model"] == "genie_obs"]
score_models_genie_standardscaler_tf = score_models[score_models["model"] == "genie_obs_tf"]
...
fig.savefig("../results_softODE_sergio/genie_obs.png", bbox_inches = "tight") 
...
fig.savefig("../results_softODE_sergio/genie_obs_tf.png", bbox_inches = "tight") 
