...
seeds = [0,1,2]
bandwidths = [0.1, 1, 10]
truncate_params = [15, 30, 100]

ntimes = 1000
nsamples = 1
path = "../../data/GGM_bifurcate/"
max_iters = 500
for kt in [kts[eval(sys.argv[1])]]:
    for interval in [intervals[eval(sys.argv[2])]]:
        for (ngenes, ntfs) in [(200, 20)]:
            for seed in [seeds[eval(sys.argv[3])]]:
                ...
                print("ntimes_" + str(ntimes) + "_interval_" + str(interval) + "_ngenes_" + str(ngenes) + "_" + str(seed))
                ...
                for bandwidth in [bandwidths[eval(sys.argv[4])]]:
                    for truncate_param in [truncate_params[eval(sys.argv[5])]]:
                        ...
                        for lamb in [0.001, 0.01, 0.1]:
                            print("bandwidth: " + str(bandwidth) + ", truncate_param: " + str(truncate_param) + ", lamb" + str(lamb))
                            start_time = time.time() 
