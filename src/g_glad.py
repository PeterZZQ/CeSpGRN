...
class G_glad():
    ...
    def train(self, t, max_iters = 500, n_intervals = 10, lamb = 2.1e-4, rho = 1, beta = 0, theta_init_offset = 0.1):
        ...
        # check to make sure w sum up to 1
        ...
        loss2 = Z.abs().sum()
        loss3 = torch.norm(self.mask * Z)
        print("Initial: loss1: {:.5f}, loss2: {:.5f}, loss3: {:.5f}".format(loss1.item(), loss2.item(), loss3.item()))
 
        while(it < max_iters):  
            Y = empir_cov_ave/rho - Z            
            theta = - 0.5 * Y + torch_sqrtm(Y.t() @ Y * 0.25 + I/rho + 1e-6)
            Z_pre = Z.detach().clone()
            Z = torch.sign(theta) * torch.max((rho * theta.abs() - lamb)/(rho + beta * self.mask), zero)
            ...
            # stopping criteria
            residual = rho * (Z_pre - Z).pow(2).sum().sqrt()
            # free up memory
            del Z_pre
            if residual < 1e-6:
                break
...
class G_glad_batch():
    ...
    def train(self, max_iters = 500, n_intervals = 10, lamb = 2.1e-4, rho = 1, beta = 0, theta_init_offset = 0.1):
        ...
        while(it < max_iters):   
            ...
            thetas = - 0.5 * Y + torch.stack([torch_sqrtm(mat) for mat in (torch.transpose(Y,1,2) @ Y * 0.25 + I/rho + 1e-6)])
            Z = torch.sign(thetas) * torch.max((rho * thetas.abs() - lamb)/(rho + beta * self.mask), torch.Tensor([0]).to(device))
            Z_pre = Z.detach().clone()
