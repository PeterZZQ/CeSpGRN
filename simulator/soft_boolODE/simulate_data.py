...
X_umap = pca_op.fit_transform(X)
ax.scatter(X_umap[:, 0], X_umap[:, 1], s = 5, c = pt)
fig.savefig(path + "true_count_plot.png", bbox_inches = "tight")
...
X_umap = pca_op.fit_transform(X)
ax.scatter(X_umap[:, 0], X_umap[:, 1], s = 5, c = pt)
fig.savefig(path + "observed_count_plot.png", bbox_inches = "tight")

np.save(path + "true_count.npy", results["true count"].T)
np.save(path + "obs_count.npy", results["observed count"].T)
np.save(path + "pseudotime.npy", results["pseudotime"])
np.save(path + "GRNs.npy", GRNs)
